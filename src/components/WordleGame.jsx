...
function Tile({ char, color }) {
  ...
  return (
    <div className={`w-14 h-14 flex items-center justify-center rounded-lg text-2xl font-extrabold border-2 transition-all duration-300 ${bgClass}`}>
...
export default function WordleGame({ game, wordListReady }) {
  ...
  for (let i = 0; i < 6; i++) {
    if (i < guesses.length) {
      ...
      rows.push(
        <div key={i} onClick={refocus} className="cursor-text">
          <WordRow word={inputValue} />
        </div>
      );
    } else {
