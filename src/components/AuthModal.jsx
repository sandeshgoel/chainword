...
export default function AuthModal({ open, onClose, user, userProfile, lastUser, onSignIn, onSignOut }) {
  return (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50">
            ...
            <div className="min-w-0">
              <p className="font-semibold text-gray-900 dark:text-white truncate">{lastUser.displayName}</p>
              <p className="text-xs text-amber-500 font-medium">Not signed in</p>
            </div>
          </div>
          <button
            onClick={() => { onSignIn(lastUser.email); onClose(); }}
          ...
          >
            ...
            Sign in as {lastUser.displayName?.split(' ')[0] ?? 'you'}
          </button>
          <button
            onClick={() => { onSignIn(null); onClose(); }}
            className="w-full py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors"
          >
            Sign in as someone else
          </button>
          <p className="text-xs text-center text-gray-400 dark:text-gray-500 -mt-2">
            Progress is saved locally until you sign in.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
